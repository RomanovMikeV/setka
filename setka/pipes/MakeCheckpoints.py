...
class MakeCheckpoints(Pipe):
    ...
    def on_init(self):
        self.log_dir = os.path.join(self.log_dir, 'logs', self.name, str(self.trainer.creation_time))
    ...
    def before_epoch(self):
        ...
        if hasattr(self.trainer, "_mode") and (self.trainer._mode == 'train'):
            ...
            torch.save({'trainer': self.trainer},
                       os.path.join(self.log_dir, 'checkpoints', self.name + '_latest.pth.tar'))

            torch.save(self.trainer._model.state_dict(),
                       os.path.join(self.log_dir, 'checkpoints', self.name + '_weights_latest.pth.tar'))

            if is_best:
                torch.save({'trainer': self.trainer},
                        os.path.join(self.log_dir, 'checkpoints', self.name + '_best.pth.tar'))

                torch.save(self.trainer._model.state_dict(),
                        os.path.join(self.log_dir, 'checkpoints', self.name + '_weights_best.pth.tar'))
