...
def test_ComputeMetrics():
    ...
    # assert(numpy.abs(trainer._metrics['train']['tensor_acc'] - 0.20) < 0.02)
    # assert(numpy.abs(trainer._metrics['valid']['tensor_acc'] - 0.19) < 0.02)
    # assert(numpy.abs(trainer._metrics['train']['const'] - 1.0) < 0.02)
    # assert(numpy.abs(trainer._metrics['valid']['const'] - 1.0) < 0.02)
    #
    # assert(numpy.abs(trainer._metrics['train']['tensor_loss'] - 2.18) < 0.02)
    # assert(numpy.abs(trainer._metrics['valid']['tensor_loss'] - 2.17) < 0.02)
        ...
        setka.pipes.OneStepOptimizers(
            [
                setka.base.Optimizer(
                    ...
                    momentum=0.9,
                    weight_decay=5e-4,
                    iter_schedulers=[setka.base.Scheduler(torch.optim.lr_scheduler.CyclicLR, base_lr=1.0e-3, max_lr=1.0e-1)],
                    epoch_schedulers=[setka.base.Scheduler(
                        torch.optim.lr_scheduler.ReduceLROnPlateau,
                        monitor=lambda: trainer._metrics['valid']['tensor_acc'][0] / trainer._metrics['valid']['tensor_acc'][1],
                        mode='max'
                    )],
                    # schedulers={
                    #     'batch': [cyclic_lr],
